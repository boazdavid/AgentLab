...
- ``render_index_preamble(wiki_dir)`` — build the browser-framed knowledge
  ...
  ``preamble_index_batch.md`` at the ``{{INDEX_MD}}`` placeholder.
...
_PREAMBLE_PATH = Path(__file__).with_name("preamble_index_batch.md")
...
def render_index_preamble(wiki_dir) -> str:
    ...
    Reads ``preamble_index_batch.md`` (next to this module) and the built wiki's
...
def select_pairs(tasks, n_train: int, n_test: int, tier: str = "l1"):
    """Filter ``split_pairs(n_train, n_test, tier=tier)`` to matching task_ids.
    ...
    ``tier`` selects the WorkArena tier ("l1"/"l2"); it defaults to "l1" so the
    existing signature behaves as before.
    Returns ``(train_pairs, test_pairs)`` preserving split_pairs' ordering.
    """
    subs = [s for s in tasks if s]
    train, test = split_pairs(n_train=n_train, n_test=n_test, tier=tier)
